...
class SelectRequestHandler {
  ...
  static async selectRequest(requestBody) {
    console.log("----Select req started----");
    ...
    try {
      ...
      // Construct the correct endpoint URL based on whether bpp_uri ends with '/'
      const selectEndpoint = requestBody.context.bpp_uri.endsWith("/")
        ? `${requestBody.context.bpp_uri}select`
        : `${requestBody.context.bpp_uri}/select`;
      ...
      const response = await axios.post(selectEndpoint, requestBody, {
        headers: {
...
module.exports = {
  selectRequest: SelectRequestHandler.selectRequest.bind(SelectRequestHandler),
};
